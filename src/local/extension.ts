...
export function createPiRecipesExtension(
  ...
  const localRunController: AgentRunController = {
    ...
    async wait(id, signal) {
      const live = childRuns.get(id);
      if (live) return controllerSummary(await waitForRun(live, signal));
      const archived = archivedRuns.get(id);
      if (archived) return controllerSummary(archived);
    ...
    async rehydrate() {
      if (!state) return 0;
      return rehydrateChildRuns(state.cwd);
    },
  };
