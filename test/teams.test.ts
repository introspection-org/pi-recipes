...
describe("Teams channel tools", () => {
  ...
  it("keeps a successful reply when bookkeeping is cancelled", async () => {
    ...
    const result = (await pi.tools.get("channel_reply")?.execute(
      ...
      undefined,
      undefined as never,
    )) as { details: { ref: string; bridge_recorded: boolean; bridge_error?: string } };

    expect(result.details.ref).toMatch(/^msg_/);
    expect(result.details.bridge_recorded).toBe(false);
    expect(result.details.bridge_error).toContain("aborted");
    expect(calls).toBe(2);
  });
