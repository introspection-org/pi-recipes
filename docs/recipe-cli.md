...
It chooses the package manager from `packageManager` when present, otherwise from
the lockfile. Local recipe installs run the same dependency install in the local
directory so extension imports resolve during local development.
